...
description: >-
  Before writing, running, reviewing, or approving any database migration,
  meaning adding, dropping, renaming, or altering a table, column, index, or
  constraint; backfilling or transforming existing data; or changing a schema in
  any way. Also fires whenever someone asks to migrate a database, write a
  migration, alter a schema, rename a column, drop a table, or asks whether a
  migration is safe to run. Covers the additive-only rule, the expand and
  contract pattern for a rename or a type change, and proving that a backup
  restores before any step that could lose data actually runs.
---
