...
4. **Short files.** Soft target 150 lines, hard cap 400 (**default**). Past the
...
6. **Never hardcode an environment, host, port, key, or path.** Read them from
...
7. **Validate every input and guard every route.** Every request body and
...
8. **No vague errors.** Every failure names its cause where it surfaces, never
...
9. **Every gate green before anything ships.** Order is lint, typecheck, test,
   build (**default**). The mechanism is `trunk check` plus the `node:test`
   suite, not this sentence.
10. **Never weaken or skip a gate. Fix the check instead.** A red gate blocks.
...
11. **Do not self-grade.** Route a success claim to a deterministic check when
...
13. **One focused commit per unit of work, and the message says why.** A bisect
...
15. **A rule binds everywhere, and an exception costs a written reason.** Every
...
16. **When you have to ask, ask plainly.** State the risk in one sentence
    before the question. A wall of hedged reasoning in front of a yes or no
    only delays the answer.

## Keeping this file honest

17. **Anything you were corrected on twice gets written down.** As a skill
